    ...
    <div className="flex h-screen bg-bg overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto bg-bg">
        ...
        <div className="p-8">
          ...
          {/* Two Column Layout */}
          <div className="grid grid-cols-1.4fr-1fr gap-5 mb-5">
            {/* Revenue Chart Card */}
            <Card
            ...
            >
              <div className="mb-3.5">
                <span className="font-bebas text-4xl text-accent">₱98,450</span>
                <span className="text-xs text-success ml-2">↑ 14% vs March</span>
              </div>
              {/* Bar Chart */}
              <div className="flex items-end gap-2 h-24 px-1">
                {[40, 55, 48, 62, 70, 58, 75, 80, 68, 85, 78, 100].map((height, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    ></div>
                    <span className={`font-mono text-9px ${idx === 11 ? 'text-accent' : 'text-muted'}`}>
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div className="relative w-24 h-24 flex-shrink-0">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <div className="font-bebas text-2xl text-text">248</div>
                  ...
                  <div className="flex flex-col gap-2 justify-center">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span>Basic — 44% (109)</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <div className="w-2.5 h-2.5 rounded-full bg-accent3 flex-shrink-0"></div>
                      <span>Premium — 32% (79)</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <div className="w-2.5 h-2.5 rounded-full bg-accent2 flex-shrink-0"></div>
                      <span>VIP — 12% (30)</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <div className="w-2.5 h-2.5 rounded-full bg-muted flex-shrink-0"></div>
                      <span>Walk-in — 12% (30)</span>
                    </div>
                  </div>
                </div>
              </Card>
              ...
              <Card title="Live Activity" action={<div className="w-2 h-2 bg-accent2 rounded-full animate-pulse"></div>} className="flex-1">
                <div className="space-y-0">
                  {activityItems.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-3 py-2.75 border-b border-border/40 last:border-b-0">
                      <div
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-10px text-muted mt-0.5">{item.time} {item.detail && '· ' + item.detail}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <Card
            ...
            action={
              <button className="px-3 py-1.5 bg-transparent border border-border text-muted text-xs font-semibold rounded-lg hover:text-text hover:border-muted transition-all">
                View All
              </button>
            }
          >
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="text-left font-mono text-9px text-muted uppercase tracking-widest pb-3 px-4 border-b border-border">Member</th>
                    <th className="text-left font-mono text-9px text-muted uppercase tracking-widest pb-3 px-4 border-b border-border">Plan</th>
                    <th className="text-left font-mono text-9px text-muted uppercase tracking-widest pb-3 px-4 border-b border-border">Status</th>
                    <th className="text-left font-mono text-9px text-muted uppercase tracking-widest pb-3 px-4 border-b border-border">Expiry</th>
                    <th className="text-left font-mono text-9px text-muted uppercase tracking-widest pb-3 px-4 border-b border-border">Attendance</th>
                    <th className="text-left font-mono text-9px text-muted uppercase tracking-widest pb-3 px-4 border-b border-border">Actions</th>
                  </tr>
                </thead>
                <tbody>
